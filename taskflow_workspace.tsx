...
// 5 Distinct Themes: Bright, Bubbly, Colourful, Dark, and Professional
const THEMES = {
  bright: {
    id: 'bright',
    name: 'Bright',
...
export default function App() {
  ...
  const activeTheme = THEMES[currentTheme] || THEMES.bright;
  ...
  const getPriorityBadge = (priority) => {
    switch (priority) {
      case 'URGENT':
        return (
          <span className="px-2 py-0.5 text-[10px] font-bold rounded-md bg-rose-500 text-white flex items-center gap-1 shadow-xs">
      ...
      case 'MEDIUM':
        return (
          <span className="px-2 py-0.5 text-[10px] font-bold rounded-md bg-blue-100 text-blue-700 border border-blue-200">
            MEDIUM
          </span>
        );
      case 'LOW':
  ...
  return (
    <div className={`min-h-screen ${activeTheme.bg} transition-colors duration-300 font-sans pb-12`}>
      ...
      <header className={`sticky top-0 z-30 ${activeTheme.header} shadow-xs`}>
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-16 flex items-center justify-between gap-4">
          ...
          {/* Controls: Theme Switcher, Google Calendar & Postgres */}
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 items-start">
            {[
              ...
              return (
                <div
                  key={col.id}
                  className={`p-3.5 ${activeTheme.radius} ${activeTheme.column} border min-h-[450px] flex flex-col gap-3`}
                >
                  {/* Column Header */}
